...
export const tools = [
  ...
  },
  {
    ...
    description: "Edit an existing reminder's title, body, or schedule. Only pass the fields that are changing.",
    input_schema: {
      type: "object",
      properties: {
        id: { type: "string" },
        ...
        publishAt: {
          type: "string",
          description: "ISO 8601 date-time to reschedule this reminder to.",
        },
      },
      required: ["id"],
    },
  ...
  {
    name: "update_event",
    description: "Edit an existing calendar event. Only pass the fields that are changing.",
    input_schema: {
      type: "object",
      properties: {
        id: { type: "string" },
  ...
  return async function execute(name, input) {
    switch (name) {
      ...
      case "update_reminder": {
        ...
        if (typeof input.body === "string") updates.body = input.body || null;
        if (input.publishAt) {
          const publishAt = new Date(input.publishAt);
          if (Number.isNaN(publishAt.getTime())) return { error: "publishAt isn't a valid date/time" };
          updates.publish_at = publishAt.toISOString();
          updates.week_of = publishAt.toISOString().slice(0, 10);
        }
      ...
      case "update_event": {
        const updates = {};
        if (typeof input.title === "string") updates.title = input.title;
        if (typeof input.description === "string") updates.description = input.description || null;
        if (typeof input.location === "string") updates.location = input.location || null;
        if (typeof input.allDay === "boolean") updates.all_day = input.allDay;
        if (input.startAt) {
          const startAt = new Date(input.startAt);
          if (Number.isNaN(startAt.getTime())) return { error: "startAt isn't a valid date/time" };
          updates.start_at = startAt.toISOString();
        }
        if (input.endAt) {
          const endAt = new Date(input.endAt);
          if (Number.isNaN(endAt.getTime())) return { error: "endAt isn't a valid date/time" };
