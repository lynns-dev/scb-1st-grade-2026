...
export function buildDigestEmail({ reminders, events, appUrl }) {
  ...
  const eventRows = events.length
    ? events
      ...
      <tr>
        <td style="padding:10px 0;border-bottom:1px solid #eef0f8;">
          <div style="font-weight:600;color:#1c2140;">${escapeHtml(e.title)}</div>
          <div style="color:#5b6178;font-size:14px;margin-top:2px;">${formatEventDate(e.start_at, e.all_day)}</div>
        </td>
      </tr>`
        )
        .join("")
  ...
  <div style="font-family:-apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif;max-width:480px;margin:0 auto;padding:24px;">
    ...
    ${
      appUrl
        ? `<p style="margin-top:28px;"><a href="${escapeHtml(appUrl)}" style="background:#4a63f2;color:#fff;text-decoration:none;padding:10px 18px;border-radius:10px;font-size:14px;font-weight:600;display:inline-block;">Open the app</a></p>`
        : ""
    }
  </div>`;
